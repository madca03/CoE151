...
class P2P:
  ...
  def connect(self, ip, port):
    ...
    message = '*** You are now chatting with %s ***\n' % (self.username)
  ...
  def _message_reader(self):
    while not self.reader_thread.stopped():
      try:
        ...
        if data_cmd[1] == '/changename':
          if (data_cmd[2]):
            self.set_peer_username(data_cmd[2])
            continue
        elif data_cmd[1] == '/quit':
          print('*** Closing connection ***')

          # if the peer that receives a quit signal is not the one who initiated the closing of connection
          # then this peer should send a quit signal to the initiator peer.
